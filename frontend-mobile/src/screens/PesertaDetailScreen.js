...
export default function PesertaDetailScreen({ route, navigation }) {
  ...
  try {
    ...
    setPeserta(response.data.data); // ← FIX: tambah .data lagi
  } catch (error) {
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.value}>
          {peserta.tanggalLahir
            ? new Date(peserta.tanggalLahir).toLocaleDateString("id-ID")
            : "-"}
        </Text>
        ...
        <Text style={styles.value}>{peserta.nama_kabkot || "-"}</Text>
        {/* ============================================ */}
